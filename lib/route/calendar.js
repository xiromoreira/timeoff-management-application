...
router.post('/bookleave/', async function (req, res) {
  ...
  try {
    ...
    const [users, company, valid_attributes] = await Promise.all([
      req.user.promise_users_I_can_manage(),
      req.user.get_company_with_all_leave_types(),
      get_and_validate_leave_params({ req })
    ])
    ...
    let leave = await createNewLeave({
    ...
    await (new EmailTransport()).promise_leave_request_emails({ leave })
...
router.get('/teamview/', async (req, res) => {
  ...
  try {
    const [team_view_details, company] = await Bluebird.all([
