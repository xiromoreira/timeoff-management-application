...
module.exports = function() {
  ...
  router.post('/login', async function (req, res) {
    const user = await check_login(req.body)
    if (!user) {
  ...
  router.post('/register', async function (req, res) {
    ...
    try {
      ...
      await EmailTransport.send_email_user(user, 'registration_complete', {})
  ...
  router.post('/reset-password/', function(req, res){
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      ...
      .then(function(user){
        return EmailTransport.promise_reset_password_email({
          user : user,
        });
      })
      .then(function(){
