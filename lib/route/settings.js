...
router.get('/company/authentication/', async (req, res) => {
  ...
  const auth_config = await company.get('auth_config');
...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .then(function(company){
      ...
      var methods = []
      if (req.body.local_auth_enabled) methods.push({ method: 'local' })
      if (parameters.ldap_auth_enabled) methods.push({ method: 'ldap' })
      company.set('auth_config', methods)

      if (!parameters.ldap_auth_enabled) return company.save();
