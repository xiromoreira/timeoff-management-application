...
router.get('/company/authentication/', async (req, res) => {
  const company = req.user.getCompany();
  const auth_config = (await company.get('auth_config')) || [{ method: 'local' }];

  res.locals.custom_java_script.push('/js/company-authentication.js');

  const imap_config = auth_config?.find(m => m.method === 'imap')?.config;
  if (imap_config)
    imap_config.security = imap_config.tls ? 'tls' : imap_config.autotls ? 'starttls' : ''

  res.render('settings_company_authentication', {
    auth_domain: await company.get('auth_domain'),
    local_config: auth_config.some(m => m.method === 'local'),
    ldap_config: auth_config.find(m => m.method === 'ldap')?.config,
    imap_config,
  });
});

router.post('/company/authentication/', async (req, res) => {
  const company = await req.user.getCompany()

  const config = get_and_validate_auth_configuration(req)

  await company.update({
    auth_domain: req.body['auth_domain'] || null,
    auth_config: config,
  })
  ...
  if (req.session.flash_has_errors()) {
    ...
    req.session.flash_message('Auth configuration was updated');
    return res.redirect_with_session('/settings/company/authentication/');
  }
});
...
const get_and_validate_auth_configuration = (req) => {
  const getBoolean = field => validator.toBoolean(req.body[field])
  const getString = field => validator.trim(req.body[field])

  const local_enabled = getBoolean('local_auth_enabled')
  const ldap_enabled = getBoolean('ldap_auth_enabled')
  const imap_enabled = getBoolean('imap_auth_enabled')
  const config = []

  if (local_enabled)
    config.push({ method: 'local' })

  if (ldap_enabled) {
    const ldap_config = {
      url: getString('url'),
      binddn: getString('binddn'),
      bindcredentials: getString('bindcredentials'),
      searchbase: getString('searchbase'),
      allow_unauthorized_cert: getBoolean('allow_unauthorized_cert')
    }
    if (!validator.matches(ldap_config.url, /^ldaps?:\/\/[a-z0-9\.\-]+:\d+$/i)) {
      req.session.flash_error(
  ...
  if (imap_enabled) {
    const configImap = {
      host: getString('imap_host'),
      port: Number(req.body['imap_port']), // TODO implement validator for integer
      tls: req.body['imap_security'] === 'tls'
    }
    if (req.body['imap_security'] === 'starttls')
      configImap.autotls = 'required'
    config.push({ method: 'imap', config: configImap })
  }
