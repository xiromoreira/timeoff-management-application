...
const { hash_password } = require('../utils/auth');
  ...
  req.user
    ...
    .then((company) => {
      res.render('user_add', {
        company: {
          name: company.name,
          hasLocalPasswords: company.hasLocalPasswords(),
        },
...
router.post('/add/', async (req, res) => {
  ...
  const company = await req.user.get_company_for_add_user();

  new_user_attributes = get_and_validate_user_parameters({
    req: req,
    item_name: 'user',
    departments: company.departments,
    require_password: (company.hasLocalPasswords()),
  });
  ...
  try {
    ...
    const new_user = await UserImporter.add_user({
    ...
    await EmailTransport.promise_add_new_user_email({
      company,
      admin_user: req.user,
      new_user: new_user,
    })
  ...
  Promise.try(function () {
    ...
    .then(function (company) {
      ...
      if (new_user_attributes.password) {
        new_user_attributes.password = hash_password(
