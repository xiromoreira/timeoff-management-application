...
const
  ...
  Models    = require('./db');
const { hash_password } = require('../route/utils/auth');
...
async function add_user(args) {
  ...
  attributes.password     = await hash_password(args.password);
  ...
  await validate_email_to_be_free({ email : args.email })
